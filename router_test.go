...
func TestMethodNotAllowedFallthrough(t *testing.T) {
	...
	checkRoute := func(method, path, expectedMethod, expectedPath string,
		...
		matchedMethod = ""
		matchedPath = ""
		matchedParams = make(map[string]string) // Ensure it's always initialized
		...
		// Debugging logs
		t.Logf("Testing %s %s -> Expected: method=%s, path=%s, code=%d, params=%+v",
			method, path, expectedMethod, expectedPath, expectedCode, expectedParams)
		...
		if w.Code == http.StatusOK {
			if matchedMethod != expectedMethod || matchedPath != expectedPath {
	...
	// Run tests
	checkRoute("GET", "/apple/banana/cat", "GET", "/apple/banana/cat", http.StatusOK, map[string]string{})
	checkRoute("POST", "/apple/banana/cat", "POST", "/apple/banana/:abc", http.StatusOK,
		map[string]string{"abc": "cat"})
	checkRoute("POST", "/apple/banana/dog", "POST", "/apple/banana/:abc", http.StatusOK,
		map[string]string{"abc": "dog"})

	// Wildcards should be checked before catchalls
	checkRoute("DELETE", "/apple/banana", "DELETE", "/apple/:seed", http.StatusOK,
		map[string]string{"seed": "banana"})
	checkRoute("DELETE", "/apple/banana/cat", "DELETE", "/apple/*path", http.StatusOK,
		map[string]string{"path": "banana/cat"})

	checkRoute("POST", "/apple/ban/def", "POST", "/apple/ban/def", http.StatusOK, map[string]string{})
	checkRoute("OPTIONS", "/apple/ban/def", "OPTIONS", "/apple/*path", http.StatusOK,
		map[string]string{"path": "ban/def"})
	checkRoute("GET", "/apple/ban/def", "", "", http.StatusMethodNotAllowed, map[string]string{})

	// Always fallback to the matching handler
	checkRoute("OPTIONS", "/apple/banana/cat", "OPTIONS", "/apple/*path", http.StatusOK,
		map[string]string{"path": "banana/cat"})
	checkRoute("OPTIONS", "/apple/bbbb", "OPTIONS", "/apple/*path", http.StatusOK,
		map[string]string{"path": "bbbb"})

	// PATCH requests should be rejected
	checkRoute("PATCH", "/apple/banana/cat", "", "", http.StatusMethodNotAllowed, map[string]string{})
	checkRoute("PATCH", "/apple/potato", "", "", http.StatusMethodNotAllowed, map[string]string{})

	// 404 cases
	checkRoute("GET", "/abc", "", "", http.StatusNotFound, map[string]string{})
	checkRoute("OPTIONS", "/apple", "", "", http.StatusMovedPermanently, map[string]string{})
}
